...
The system SHALL validate all Foreman artifact categories (config, specs, archives, changes, graphs, templates) when `foreman doctor` is run without arguments.

#### Scenario: Full project health check
- **GIVEN** a Foreman project with `.foreman/` initialized
- **WHEN** the user runs `foreman doctor`
...
- **GIVEN** a Foreman project with an active change named "my-feature"
- **WHEN** the user runs `foreman doctor my-feature`
...
- **GIVEN** an archive directory with `.foreman.yaml` (status: archived), `proposal.md`, `design.md`, `tasks.md`, and `summary.md`
...
The system SHALL validate that `.foreman/config.yaml` exists and contains required sections.

#### Scenario: Valid config
- **GIVEN** a `.foreman/config.yaml` with models, execution, spec, and graph sections
- **WHEN** the doctor checks config
- **THEN** the check SHALL pass

#### Scenario: Missing config
- **GIVEN** no `.foreman/config.yaml` file exists
- **WHEN** the doctor checks config
- **THEN** the check SHALL fail with an error

#### Scenario: Missing required section
- **GIVEN** a `.foreman/config.yaml` missing the `execution` section
- **WHEN** the doctor checks config
...
- **WHEN** the user runs `foreman doctor --fix`
...
- **WHEN** node directories exist for all three in `.foreman/nodes/<change>/`
- **THEN** the cross-ref check SHALL pass

#### Scenario: Orphaned node directory
- **GIVEN** a node directory `.foreman/nodes/<change>/orphan-node/` that is not in graph.yaml
