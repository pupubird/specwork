...
Every Foreman CLI JSON response SHALL include a `next_action` field that tells the agent exactly what to do next. This enables "gradual reveal" — workflow knowledge moves from the agent's memory (SKILL.md) into the state machine's responses.

---

### Requirement: Structured Next Action in JSON Responses

Every JSON response from `foreman go`, `foreman node start`, `foreman node complete`, `foreman node fail`, `foreman node escalate`, and `foreman node verify` SHALL include a `next_action` field.
...
#### Scenario: foreman go with ready nodes returns spawn action

GIVEN a change with nodes in `pending` state whose dependencies are met
WHEN the agent runs `foreman go <change-name>`
...
#### Scenario: foreman node start returns subagent execution action

GIVEN a node has been started and is now `in_progress`
WHEN the agent runs `foreman node start <change-name> <node-id>`
THEN the JSON response SHALL include `next_action.command` that instructs spawning the appropriate subagent
AND `next_action.on_pass` SHALL be `"foreman node complete <node-id> --summary '<L0>'"` (with placeholder)
AND `next_action.on_fail` SHALL be `"foreman node fail <node-id> --reason '<error>'"` (with placeholder)

#### Scenario: foreman node complete returns go-again action

GIVEN a node has been marked complete
WHEN the agent runs `foreman node complete <change-name> <node-id> --summary '<L0>'`
THEN the JSON response SHALL include `next_action.command` equal to `"foreman go <change-name>"`
AND `next_action.description` SHALL indicate running go again to pick up the next batch

#### Scenario: foreman node verify PASS returns complete action

GIVEN a verifier has run and all checks passed
WHEN the agent runs `foreman node verify <change-name> <node-id> --result pass`
THEN the JSON response SHALL include `next_action.command` that instructs completing the node
AND `next_action.on_pass` SHALL be `"foreman node complete <node-id> --summary '<L0>'"` (with placeholder)

#### Scenario: foreman node verify FAIL returns fail action

GIVEN a verifier has run and one or more checks failed
WHEN the agent runs `foreman node verify <change-name> <node-id> --result fail`
THEN the JSON response SHALL include `next_action.command` that instructs failing the node
AND `next_action.on_fail` SHALL be `"foreman node fail <node-id> --reason '<error>'"` (with placeholder)

#### Scenario: foreman node fail with retries remaining returns respawn action

GIVEN a node has failed and has retries remaining
WHEN the agent runs `foreman node fail <change-name> <node-id> --reason '<error>'`
...
#### Scenario: foreman node fail with no retries returns escalate action

GIVEN a node has failed and has exhausted all retries
WHEN the agent runs `foreman node fail <change-name> <node-id> --reason '<error>'`
THEN the JSON response SHALL include `next_action.command` equal to `"foreman node escalate <node-id>"`
AND `next_action.description` SHALL mention reporting to the user for manual intervention

#### Scenario: foreman node escalate returns user-facing suggestion

GIVEN a node has been escalated
WHEN the agent runs `foreman node escalate <change-name> <node-id>`
...
Every `next_action` object SHALL include a `context` field containing the change description from `.foreman.yaml`.

#### Scenario: context field populated from .foreman.yaml

GIVEN a change has a `.foreman.yaml` with a non-empty `description` field
WHEN the agent runs any `foreman go` or `foreman node` command
THEN `next_action.context` SHALL equal the `description` value from `.foreman.yaml`

#### Scenario: context field is empty string when .foreman.yaml is missing or has no description

GIVEN a change has no `.foreman.yaml` or the file has no `description` field
WHEN the agent runs any `foreman go` or `foreman node` command
...
Commands with branching outcomes: `foreman node start`, `foreman node verify`.
...
THEN `next_action.on_pass` SHALL be present and SHALL reference `foreman node complete`
AND `next_action.on_fail` SHALL be present and SHALL reference `foreman node fail`
...
When `foreman go` returns `status: done`, the response SHALL include `next_action.suggest_to_user` as a non-empty array of suggested follow-up actions for the user.
...
The Foreman engine SKILL.md SHALL be 60 lines or fewer after this change is implemented.
