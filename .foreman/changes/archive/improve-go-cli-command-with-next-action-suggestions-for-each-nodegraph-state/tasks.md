...
- [x] 1.2 Create `src/core/next-action.ts` with `readChangeContext(changeName: string): string` — reads `.foreman.yaml` description field, returns empty string on error
...
- [x] 2.5 Add `next_action` to `status: waiting` response branch in `go.ts` (command: `wait`, suggests re-running `foreman go`)

## 3. CLI Integration — node.ts

- [x] 3.1 Import `buildNextAction` and `readChangeContext` in `src/cli/node.ts`
- [x] 3.2 Add `next_action` to `node start` response (command: run subagent, then `foreman node complete` or `foreman node fail`)
- [x] 3.3 Add `next_action` to `node complete` response (command: `foreman go` for next batch)
...
- [x] 4.1 Rewrite `.claude/skills/foreman-engine/SKILL.md` to ≤60 lines: "read next_action, execute command, use on_pass/on_fail for branching" — remove all procedural state-machine documentation
- [x] 4.2 Trim `.claude/commands/foreman-go.md` to ≤10 lines: "run foreman go, read next_action, follow it"
...
- [x] 5.3 Write integration tests asserting `foreman go` JSON output contains `next_action` for `ready`, `done`, `blocked`, `waiting` statuses
- [x] 5.4 Write integration tests asserting `foreman node complete/fail/escalate/verify` JSON output contains `next_action` with correct branching fields
