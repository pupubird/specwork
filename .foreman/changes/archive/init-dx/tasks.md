...
- [x] 2.2 Fix post-init message (`foreman plan` not `foreman new`) and add doctor auto-run as final init step
...
- [x] 5.2 Integration test for `foreman init --force`: verify idempotent re-init overwrites existing files without error

## 6. Migration Tests

- [x] 6.1 Unit tests for `migrateOpenspec()`: spec path flattening, change directory mapping, config merge, `openspec/` deletion
- [x] 6.2 Integration test for `foreman init migrate`: fixture `openspec/` tree → assert `.foreman/specs/` and `.foreman/changes/` contents match expected mapping, `openspec/` deleted
