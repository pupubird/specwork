### Requirement: Mandatory TeamCreate for All Foreman Execution

All foreman workflow execution — including planning agent spawns and graph node execution — SHALL use TeamCreate. The use of a bare `Agent` tool call (subagent without a team) is not permitted anywhere in the foreman execution loop.
...
When the `foreman-go` command or `foreman-engine` skill executes that node
...
Given the foreman-engine skill executing any LLM node
...
Given a project initialized with `foreman init`
...
Every TeamCreate call in the foreman loop SHALL be paired with a TeamDelete call. Teams SHALL NOT be left open after a workflow batch completes.
