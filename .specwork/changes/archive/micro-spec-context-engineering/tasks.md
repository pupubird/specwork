...
- [x] 1.2 Add `MicroSpecBundle` interface to `src/types/context.ts`: `{ objective, specScenarios, parentDecisions, outOfScope, relevantFiles, successCriteria }`
...
- [x] 5.2 Implement `writeStructuredL1(root, change, nodeId, data: StructuredL1): void` in `src/core/summarizer.ts` (new module) — writes `L1-structured.json` to node artifact dir
...
- [x] 7.2 Update `src/templates/instructions/agents-specwork-summarizer.ts` to mirror the updated agent definition
...
- [x] 8.2 Add unit tests for `filterSnapshot` in `src/__tests__/core/context-assembler.test.ts` — cover scope match, empty scope, multi-glob, non-tree preservation, empty result
...
- [x] 8.4 Add unit tests for `getStructuredL1` and `expandValidate` in `src/__tests__/core/context-assembler.test.ts`
