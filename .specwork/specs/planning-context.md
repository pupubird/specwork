...
Before spawning the planner agent, the `specwork-plan` command SHALL assemble a compact codebase context block and pass it as input to the agent. The context SHALL include: (a) a listing of files in `.specwork/specs/` with their requirement headers only, (b) the current environment snapshot (file tree + dependencies), and (c) relevant source file paths.
...
Given a change created via `specwork plan "<description>"`
When the `specwork-plan` command prepares to spawn the planner in research phase
Then the command SHALL read `.specwork/specs/` and extract only `### Requirement:` header lines from each spec file
...
And the planner agent SHALL NOT need to re-read `.specwork/specs/` on its own

#### Scenario: Planner receives pre-assembled context in YOLO mode
Given a change created via `specwork plan "<description>" --yolo`
When the `specwork-plan` command prepares to spawn the planner in YOLO phase
...
Given a project with multiple spec files in `.specwork/specs/`
...
Given a project with no files in `.specwork/specs/`
When the planning context is assembled
...
When a planner agent receives a pre-assembled `<planning-context>` block, it SHALL use that block as the authoritative source for existing specs and file structure. It SHALL NOT re-read `.specwork/specs/` unless it needs detail beyond what the headers provide.
...
And the agent SHALL NOT make additional reads of `.specwork/specs/` files for the purpose of listing capabilities
...
The `specwork-plan` slash command SHALL use TeamCreate to manage the planner agent lifecycle, regardless of whether the flow is brainstorm or YOLO mode.

#### Scenario: Brainstorm planning uses a team
Given a change in brainstorm mode
When `specwork-plan` spawns the planner for the research phase
...
When `specwork-plan` spawns the planner for the YOLO phase
