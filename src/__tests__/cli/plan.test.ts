...
describe('specwork plan', () => {
  ...
  it('creates the change directory with all template files', () => {
    ...
    expect(fs.existsSync(path.join(out.path, '.specwork.yaml'))).toBe(true);
  });

  it('writes description into proposal.md', () => {
    runSpecwork(dir, 'plan "Add JWT authentication" --json');
    const proposalPath = path.join(dir, '.specwork', 'changes', 'add-jwt-authentication', 'proposal.md');
    const content = fs.readFileSync(proposalPath, 'utf-8');
    expect(content).toContain('Add JWT authentication');
  });

  it('sets .specwork.yaml status to planning', () => {
    runSpecwork(dir, 'plan "Add JWT authentication" --json');
    const metaPath = path.join(dir, '.specwork', 'changes', 'add-jwt-authentication', '.specwork.yaml');
  ...
  it('slugifies description into kebab-case change name', () => {
    const result = runSpecwork(dir, 'plan "Fix the Login Bug" --json');
    const out = JSON.parse(result.stdout);
    expect(out.change).toBe('fix-the-login-bug');
  });

  it('accepts --name to override auto-generated change name', () => {
    const result = runSpecwork(dir, 'plan "Add JWT authentication" --name auth-jwt --json');
    const out = JSON.parse(result.stdout);
    expect(out.change).toBe('auth-jwt');
  });

  it('fails if change name already exists', () => {
    runSpecwork(dir, 'plan "First change" --name my-change --json');
    const result = runSpecwork(dir, 'plan "Second change" --name my-change --json');
    expect(result.exitCode).not.toBe(0);
    expect(result.stderr).toContain('already exists');
  });

  it('creates change successfully without --json flag', () => {
    const result = runSpecwork(dir, 'plan "Add JWT authentication"');
    expect(result.exitCode).toBe(0);
    // Verify the change was actually created on disk
    const changeDir = path.join(dir, '.specwork', 'changes', 'add-jwt-authentication');
  ...
  it('sets yolo mode with --yolo flag', () => {
    const result = runSpecwork(dir, 'plan "Add JWT authentication" --yolo --json');
    expect(result.exitCode).toBe(0);
    const out = JSON.parse(result.stdout);
    expect(out.mode).toBe('yolo');
  });

  it('sets .specwork.yaml mode field in yolo mode', () => {
    runSpecwork(dir, 'plan "Add JWT authentication" --yolo --json');
    const metaPath = path.join(dir, '.specwork', 'changes', 'add-jwt-authentication', '.specwork.yaml');
    const meta = parseYaml(fs.readFileSync(metaPath, 'utf-8')) as Record<string, unknown>;
...
describe('specwork go', () => {
  ...
  it('fails if change does not exist', () => {
    const result = runSpecwork(dir, 'go nonexistent --json');
    expect(result.exitCode).not.toBe(0);
  ...
  it('fails if no tasks.md and no graph', () => {
    ...
    fs.unlinkSync(path.join(dir, '.specwork', 'changes', 'my-change', 'tasks.md'));
    const result = runSpecwork(dir, 'go my-change --json');
    expect(result.exitCode).not.toBe(0);
  ...
  it('reports done when all nodes are complete', () => {
    ...
    const stateFp = path.join(dir, '.specwork', 'graph', 'my-change', 'state.yaml');
  ...
  it('outputs human-readable text without --json flag', () => {
    ...
    const result = runSpecwork(dir, 'go my-change');
    expect(result.exitCode).toBe(0);
