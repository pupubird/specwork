...
function markNodeComplete(dir: string, change: string, nodeId: string): void {
  const sp = path.join(dir, '.foreman', 'graph', change, 'state.yaml');
...
// ══════════════════════════════════════════════════════════════════════════════
// foreman go — next_action in JSON output
// ══════════════════════════════════════════════════════════════════════════════

describe('foreman go --json next_action', () => {
  let dir: string;
  ...
  it('includes next_action with suggest for done status', () => {
    ...
    const tasksPath = path.join(dir, '.foreman', 'changes', 'my-change', 'tasks.md');
  ...
  it('includes context from .foreman.yaml description', () => {
    setupProjectWithGraph(dir);

    const result = runForeman(dir, 'go my-change --json');
    const json = JSON.parse(result.stdout);

    // .foreman.yaml was created by `foreman new`, should have a description
    expect(json.next_action.context).toBeDefined();
...
// ══════════════════════════════════════════════════════════════════════════════
// foreman node complete --json — next_action
// ══════════════════════════════════════════════════════════════════════════════

describe('foreman node complete --json next_action', () => {
  let dir: string;
  ...
  it('includes next_action pointing to foreman go for next batch', () => {
    setupProjectWithGraph(dir);
    // Start and complete the snapshot node
    runForeman(dir, 'node start my-change snapshot');

    const result = runForeman(dir, 'node complete my-change snapshot --l0 "snapshot done" --no-commit --json');
    expect(result.exitCode).toBe(0);

    const json = JSON.parse(result.stdout);
    expect(json.next_action).toBeDefined();
    expect(json.next_action.command).toMatch(/foreman go/);
...
// ══════════════════════════════════════════════════════════════════════════════
// foreman node start --json — next_action
// ══════════════════════════════════════════════════════════════════════════════

describe('foreman node start --json next_action', () => {
  let dir: string;
  ...
  it('includes next_action with on_pass and on_fail', () => {
    ...
    const result = runForeman(dir, 'node start my-change snapshot --json');
    expect(result.exitCode).toBe(0);
    ...
    expect(json.next_action.on_pass).toMatch(/foreman node complete/);
    expect(json.next_action.on_fail).toMatch(/foreman node fail/);
    expect(json.next_action.context).toBeDefined();
  });
});

// ══════════════════════════════════════════════════════════════════════════════
// foreman node verify --json — next_action
// ══════════════════════════════════════════════════════════════════════════════

describe('foreman node verify --json next_action', () => {
  let dir: string;
  ...
  it('includes next_action with on_pass referencing complete for PASS verdict', () => {
    ...
    const result = runForeman(dir, 'node verify my-change snapshot --json');
...
// ══════════════════════════════════════════════════════════════════════════════
// foreman node fail --json — next_action
// ══════════════════════════════════════════════════════════════════════════════

describe('foreman node fail --json next_action', () => {
  let dir: string;
  ...
  it('includes next_action with respawn for retries remaining', () => {
    ...
    runForeman(dir, 'node start my-change write-tests');

    const result = runForeman(dir, 'node fail my-change write-tests --reason "tests not compiling" --json');
    expect(result.exitCode).toBe(0);
...
// ══════════════════════════════════════════════════════════════════════════════
// foreman node escalate --json — next_action
// ══════════════════════════════════════════════════════════════════════════════

describe('foreman node escalate --json next_action', () => {
  let dir: string;
  ...
  it('includes next_action with suggest and suggest_to_user', () => {
    ...
    const result = runForeman(dir, 'node escalate my-change write-tests --reason "manual intervention needed" --json');
    expect(result.exitCode).toBe(0);
