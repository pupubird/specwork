...
export function makeGoCommand(): Command {
  return new Command('go')
    ...
    .action((change: string, opts: { from?: string; force: boolean }, cmd: Command) => {
      ...
      const changesDir = path.join(root, '.specwork', 'changes', change);
      ...
      if (!fs.existsSync(gp)) {
        ...
        if (!fs.existsSync(tasksPath)) {
          throw new SpecworkError(
            `No graph or tasks.md found for "${change}". Run: specwork plan "${change}" first`,
            ExitCode.ERROR
          );
        }
      ...
      if (allTerminal) {
        ...
        if (jsonMode) {
          ...
          const next_action = buildNextAction('go:done', ctx, { change });
          output({ change, status: 'done', progress, skipped, auto_generated_graph: autoGeneratedGraph, next_action }, { json: true, quiet: false });
        } else {
      ...
      if (readyNodes.length === 0) {
        ...
        if (inProgress > 0) {
          if (jsonMode) {
            const next_action = buildNextAction('go:waiting', ctx, { change });
            output({ change, status: 'waiting', progress, skipped, in_progress: inProgress, next_action }, { json: true, quiet: false });
          } else {
        ...
        if (jsonMode) {
          const next_action = buildNextAction('go:blocked', ctx, { change, blockedNodes: blocked.map(n => n.id) });
          output({
            change,
            status: 'blocked',
            progress,
      ...
      if (jsonMode) {
        const ctx = readChangeContext(root, change);
        const next_action = buildNextAction('go:ready', ctx, { change, readyNodes: ready.map(n => n.id) });
        output({ change, status: 'ready', ready, progress, skipped, auto_generated_graph: autoGeneratedGraph, next_action }, { json: true, quiet: false });
      } else {
        ...
        info(`Run: specwork status ${change}   to check progress`);
      }
    });
}
