...
export function migrateOpenspec(root: string): MigrateResult {
  ...
  if (!fs.existsSync(openspecDir)) {
    throw new ForemanError(`openspec directory not found at ${openspecDir}`);
  }
  ...
  // Migrate openspec/specs/<name>/spec.md → .foreman/specs/<name>.md
  ...
  if (fs.existsSync(specsDir)) {
    for (const entry of fs.readdirSync(specsDir, { withFileTypes: true })) {
      ...
      const dest = path.join(root, '.foreman', 'specs', `${entry.name}.md`);
  ...
  // Migrate openspec/changes/<name>/ → .foreman/changes/<name>/
  ...
  if (fs.existsSync(changesDir)) {
    for (const entry of fs.readdirSync(changesDir, { withFileTypes: true })) {
      ...
      if (fs.existsSync(proposalSrc)) {
        const proposalDest = path.join(root, '.foreman', 'changes', changeName, 'proposal.md');
      ...
      if (fs.existsSync(changeSpecsDir)) {
        for (const specEntry of fs.readdirSync(changeSpecsDir, { withFileTypes: true })) {
          ...
          if (!fs.existsSync(specFile)) continue;
          const dest = path.join(root, '.foreman', 'changes', changeName, 'specs', `${specEntry.name}.md`);
          ensureDir(path.dirname(dest));
          fs.copyFileSync(specFile, dest);
          filesTotal++;
        }
