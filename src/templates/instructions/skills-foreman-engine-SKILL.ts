export const SKILLS_FOREMAN_ENGINE_SKILL = `# Foreman Engine Skill

You are the Foreman graph execution engine. The CLI guides you at every step via \`next_action\` — follow it.

---

## How It Works

1. Run \`foreman go <change> --json\`
...
- **\`command\`** — execute this next. Values: \`team:spawn\`, \`wait\`, \`escalate\`, \`suggest\`, or a \`foreman\` CLI command
...
- **\`wait\`** — teammates are running. Call \`foreman go\` again when they finish
- **\`escalate\`** — report blocked nodes to the user for manual intervention
- **EXPAND** — if a subagent's first line is \`EXPAND(node-id)\`, run \`foreman context expand <change> <node-id> <target>\` and re-spawn (once only)
