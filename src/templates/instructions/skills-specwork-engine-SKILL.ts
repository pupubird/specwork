...
| node:start | \\\`specwork node start <change> <node> --json\\\` | run command, then \\\`specwork context assemble\\\` | Returns subagent context; sandbox is already running (auto-initialized by \\\`specwork go\\\`) |
| node:start result | \\\`subagent:spawn\\\` | spawn appropriate subagent with assembled context | Implementer, test-writer, etc. Sandbox services are live — no manual init needed |
...
| summarizer done | \\\`specwork node complete <change> <node> --json\\\` | run command (from \\\`on_pass\\\`) | Sandbox stays running across nodes — teardown happens automatically when workflow completes |
